...
class DynamicPropagation(nn.Module):
    def __init__(self, channel_in, channel_hid, N_T, incep_ker=[3, 5, 7, 11], groups=8):
        ...
        # print("channel_hid//2: ", channel_hid//2)
...
class PastNet_model(nn.Module):
    def __init__(self, 
                 ...
                 load_pred_train=0,
                 freeze_vqvae=0):
        super(PastNet_model, self).__init__()
        T, C, H, W = shape_in
        # embedding_nums = D
        # embedding_nums = K
        ...
        self.FPG_module = FPG(img_size=W,
                              patch_size=patch_size,
                              in_channels=1,
                              out_channels=1,
                              embed_dim=128,
                              input_frames=args.input_length,
                              depth=1,
                              mlp_ratio=2.,
                              uniform_drop=False,
                              drop_rate=0.,
                              ...
                              dropcls=0.)
        if load_pred_train:
            self.DST_module._vq_vae.load_state_dict(torch.load("./module/vqvae.ckpt"), strict=False)
        if freeze_vqvae:
            for p in self.DST_module._vq_vae.parameters():
                p.requires_grad = False
        self.DynamicPro = DynamicPropagation(T * (hid_T // 4), hid_T, N_T, incep_ker, groups)

    def forward(self, input_frames):
        B, T, C, H, W = input_frames.shape

        pde_features = self.FPG_module(input_frames)
        input_features = input_frames.reshape([B * T, C, H, W])
